...
- The Figma design renders an ES/EN language toggle and a USD currency dropdown on the Artwork page. Both remain intentionally non-functional ("dead UI") for this MVP — the site is Spanish/USD-only. Don't treat their presence in the UI as a signal that i18n or multi-currency logic needs implementing. The Artwork page's own "Es un regalo" checkbox (`components/galeria/artwork-options.tsx`) is separately still inert — it doesn't feed into the checkout gift option below.
...
- Update (checkout modal, Figma "Resumen de compra"): shipping address collection and the gift option (recipient name + message) are now real, not deferred — the Customer enters them at checkout, they're stored on the Order (`shippingCity`/`shippingCountry`/`shippingAddress`/`isGift`/`giftRecipientName`/`giftMessage`), and shown in both Order confirmation/notification emails and the admin/Customer Order detail views. No shipping _cost_ logic or carrier integration exists yet — only address collection and display. The accepted-payment-method badges and "Pago seguro vía Stripe" copy in that same design are still presentational only; no real payment processing is wired up.
